...
@php
  $sub = $me->activeSubscription();
  $isPrem = $sub && $sub->isCurrentlyActive();
@endphp
@if($isPrem)
  <div class="plan-card is-prem">
    ...
    <div class="plan-body">
      ...
      <b>Membre Premium ✨</b>
      <small>{{ $sub->plan?->name }}@if($sub->ends_at) · actif jusqu'au {{ $sub->ends_at->locale('fr')->isoFormat('D MMMM YYYY') }}@endif</small>
    </div>
    <a href="{{ route('subscription.mine') }}" class="btn btn-line">Gérer</a>
  </div>
@elseif(\App\Support\FeatureGate::monetizationEnabled() && ! $me->isAdminUser())
